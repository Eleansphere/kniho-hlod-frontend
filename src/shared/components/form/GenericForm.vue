...
import { useFormState } from '@/shared/composables/use-form-state';
  ...
  (newVal) => {
    if (newVal && JSON.stringify(newVal) !== JSON.stringify(state.model)) {
      ...
      for (const field of props.definition.fields) {
        if (field.type === 'date' && coerced[field.name] != null && !(coerced[field.name] instanceof Date)) {
          const parsed = new Date(coerced[field.name]);
          coerced[field.name] = isNaN(parsed.getTime()) ? null : parsed;
        }
      }
      Object.assign(state.model, coerced);
    }
  },
  { deep: true }
);
...
<template>
  <form @submit.prevent="handleSubmit" class="form-container">
    <div v-for="field in definition.fields" :key="field.name" class="form-field">
      <div v-if="!field.hidden && (!isViewOnly || state.model[field.name])" class="field-wrapper">
        <!-- Text inputs -->
