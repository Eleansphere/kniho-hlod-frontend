import type { FormDefinition, FormState } from '@/shared/components/form/types';
import { computed, reactive, watch } from 'vue';

function coerceDates<T extends Record<string, any>>(
  definition: FormDefinition<T>,
  model: Record<string, any>
): Record<string, any> {
  const result = { ...model };
  for (const field of definition.fields) {
    if (field.type === 'date' && result[field.name] != null && !(result[field.name] instanceof Date)) {
      const parsed = new Date(result[field.name]);
      result[field.name] = isNaN(parsed.getTime()) ? null : parsed;
    }
  }
  return result;
}
...
export function useFormState<T extends Record<string, any>>(
  ...
  const state = reactive<FormState<T>>({
    model: coerceDates(definition, { ...initialModel }) as T,
    errors: {},
    touched: {},
    isSubmitting: false,
    isValid: false,
  ...
  function resetForm(newModel?: T) {
    Object.assign(
      state.model as Record<string, any>,
      coerceDates(definition, { ...(newModel || initialModel) })
    );
